...
# import os
# import sys
# _root = os.path.sep.join(os.path.abspath(__file__).split(os.path.sep)[:-2])
# _root_sr = os.path.join(_root, 'StableSR')
# if __name__ == '__main__':
#     sys.path.insert(1, _root)
#     sys.path.insert(1, _root_sr)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="number of ddpm sampling count",
    )
    parser.add_argument(
        "--ddpm_steps",
        type=int,
    ...
    for item in img_list_ori:
        if os.path.exists(os.path.join(outpath, item)):
            ...
            continue
        if os.path.exists(os.path.join(outpath, 'random0', 'nofix', item)):
            img_list.remove(item)
            continue
        if '.png' not in item:
            img_list.remove(item)
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                for n in trange(niters, desc="Sampling"):
                    tic = time.time()
                    all_samples = list()
                    for count in range(opt.random_count):

                        # --- generate the outputpath for different color correction way ---
                        sample_outpath = f'{outpath}/random{count}'
                        os.makedirs(f'{sample_outpath}/nofix', exist_ok=True)
                        os.makedirs(f'{sample_outpath}/adain', exist_ok=True)
                        os.makedirs(f'{sample_outpath}/wavelet', exist_ok=True)
                        ...
                        x_samples_nofix = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)
                        for i in range(x_samples.shape[0]):
                            img_name = img_list[n*batch_size+i]
                            basename = os.path.splitext(os.path.basename(img_name))[0]
                            x_sample = 255. * rearrange(x_samples_nofix[i].cpu().numpy(), 'c h w -> h w c')
                            Image.fromarray(x_sample.astype(np.uint8)).save(
                                os.path.join(f'{sample_outpath}/nofix', f'{basename}.png'))

                        x_samples_adain = adaptive_instance_normalization(x_samples, init_image)
                        x_samples_adain = torch.clamp((x_samples_adain + 1.0) / 2.0, min=0.0, max=1.0)
                        for i in range(x_samples.shape[0]):
                            img_name = img_list[n*batch_size+i]
                            basename = os.path.splitext(os.path.basename(img_name))[0]
                            x_sample = 255. * rearrange(x_samples_adain[i].cpu().numpy(), 'c h w -> h w c')
                            Image.fromarray(x_sample.astype(np.uint8)).save(
                                os.path.join(f'{sample_outpath}/adain', f'{basename}.png'))

                        x_samples_wavelet = wavelet_reconstruction(x_samples, init_image)
                        x_samples_wavelet = torch.clamp((x_samples_adain + 1.0) / 2.0, min=0.0, max=1.0)
                        for i in range(x_samples.shape[0]):
                            img_name = img_list[n*batch_size+i]
                            basename = os.path.splitext(os.path.basename(img_name))[0]
                            x_sample = 255. * rearrange(x_samples_wavelet[i].cpu().numpy(), 'c h w -> h w c')
                            Image.fromarray(x_sample.astype(np.uint8)).save(
                                os.path.join(f'{sample_outpath}/wavelet', f'{basename}.png'))
