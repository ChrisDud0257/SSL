...
from basicsr.archs.ranksrgan_arch import RankSRGANSRResNet, Ranker_VGG12_296

@MODEL_REGISTRY.register()
class RankSRGANSimSelfMaskModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(RankSRGANSimSelfMaskModel, self).__init__(opt)
        ...
        if load_path is not None:
            if self.opt['load_mode_g'] == 'original':
                ...
                self.net_g = RankSRGANSRResNet().to(self.device)
    ...
    def init_training_settings(self):
        ...
        if load_path is not None:
            if self.opt['load_mode_r'] == 'original':
                net_r = torch.load(load_path)
                self.net_r = Ranker_VGG12_296().to(self.device)
                self.net_r.load_state_dict(net_r, strict=True)
            elif self.opt['load_mode_r'] == 'my_pretrain':
                self.net_r = build_network(self.opt['network_r'])
                self.net_r = self.model_to_device(self.net_r)
                self.print_network(self.net_r)
                param_key = self.opt['path_r'].get('param_key_r', 'params')
                self.load_network(self.net_r, load_path, self.opt['path'].get('strict_load_r', True), param_key)
        ...
        if self.ema_decay > 0:
            ...
            if load_path is not None:
                if self.opt['load_mode_g'] == 'original':
                    ...
                    self.net_g_ema = RankSRGANSRResNet().to(self.device)
        ...
        if train_opt['rank_opt'].get('use_Ranker', False):
            self.cri_rank = True
            self.logger.info(f"Rank-content loss is created.")
        else:
            self.cri_rank = False
    ...
    def train_net_g(self):
        ...
        if self.cri_rank:
            with torch.no_grad():
                l_g_rank = self.net_r(self.output)
            l_g_rank = torch.sigmoid(l_g_rank - self.opt['train']['rank_opt']['R_bias'])
            l_g_rank = torch.sum(l_g_rank)
            l_g_rank = self.opt['train']['rank_opt']['loss_weight'] * l_g_rank
            self.l_g_total += l_g_rank
            self.loss_dict['l_g_rank'] = l_g_rank
