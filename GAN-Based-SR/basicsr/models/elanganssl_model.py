...
class ELANGANSSLModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(ELANGANSSLModel, self).__init__(opt)
    ...
    def train_net_g(self):
        ...
        if self.cri_selfsim or self.cri_selfsim1:
            ...
            b_sr_list = []
            for i in range(b):
                b_mask_gt = self.gt_mask[i, :].unsqueeze(0)  # 1,1,h, w
                if self.opt['train'].get('mask_stride', 0) > 1:
                    b_mask_gt = self.mask_stride * b_mask_gt  # step sample in egde mask
                if b_mask_gt.sum() == 0:
                    pass
                else:
