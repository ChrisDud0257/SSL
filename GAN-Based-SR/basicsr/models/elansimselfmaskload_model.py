...
from basicsr.archs.elanln_arch import ELANLN
from basicsr.archs.elanlnx4_arch import ELANLNX4

def my_load_network(load_path, network, param_key = "param"):
    pretrained_dict = torch.load(load_path)[param_key]
    model_dict = {}
...
class ELANSimSelfMaskLoadModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(ELANSimSelfMaskLoadModel, self).__init__(opt)
        ...
        network_type = net_g_param.pop('type')
        if network_type == 'ELAN':
            self.net_g = ELAN(**net_g_param).to(self.device)
        elif network_type == 'ELANLN':
            self.net_g = ELANLN(**net_g_param).to(self.device)
            self.logger.info(f'The network is ELANLN')
        elif network_type == 'ELANLNX4':
            self.net_g = ELANLNX4(**net_g_param).to(self.device)
        ...
        if load_path is not None:
            ...
            if self.opt['path'].get('strict_load_g', 'False'):
                net_g = torch.load(load_path)[param_key]
                self.net_g.load_state_dict(net_g, strict=self.opt['path'].get('strict_load_g', 'False'))
            else:
                my_load_network(load_path=load_path, network=self.net_g, param_key=param_key)
