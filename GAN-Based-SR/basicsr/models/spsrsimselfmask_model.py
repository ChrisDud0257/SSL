...
def my_load_network(load_path, network):
...
class SPSRSimSelfMaskModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SPSRSimSelfMaskModel, self).__init__(opt)
        ...
        if load_path is not None:
            if self.opt['load_mode_g'] == 'original':
                if self.is_train:
                    my_load_network(load_path=load_path, network=self.net_g)
                else:
        ...
        if self.opt.get('network_d_grad', None) is not None:
            self.net_d_grad = build_network(self.opt['network_d_grad'])
            self.net_d_grad = self.model_to_device(self.net_d_grad)
            self.print_network(self.net_d_grad)

            load_path = self.opt['path'].get('pretrain_network_d_grad', None)
            if load_path is not None:
                param_key = self.opt['path'].get('param_key_d_grad', 'params')
                self.load_network(self.net_d_grad, load_path, self.opt['path'].get('strict_load_d_grad', True), param_key)

            self.use_network_d_grad = True
    ...
    def init_training_settings(self):
        self.Branch_pretrain = self.opt['train']['Branch_pretrain']
        self.Branch_init_iters = self.opt['train']['Branch_init_iters']
        ...
        if self.use_network_d_grad:
            self.net_d_grad.train()
        ...
        if self.ema_decay > 0:
            ...
            if load_path is not None:
                if self.opt['load_mode_g'] == 'original':
                    if self.is_train:
                        my_load_network(load_path=load_path, network=self.net_g_ema)
                    else:
        ...
        if train_opt.get('pixel_gradSR_opt'):   ####pixel-wise loss for gradient map in the main SR
            self.cri_pix_gradSR = build_loss(train_opt['pixel_gradSR_opt']).to(self.device)
        else:
            self.cri_pix_gradSR = None
        ...
        if train_opt.get('gan_gradSR_opt'):   ####GAN loss for gradient map in the main SR
            self.cri_gan_gradSR = build_loss(train_opt['gan_gradSR_opt']).to(self.device)
        else:
            self.cri_gan_gradSR = None

        if train_opt.get('pixel_gradBranch_opt'):   ####pixel-wise loss for gradient map in the Branch
            self.cri_pix_gradBranch = build_loss(train_opt['pixel_gradBranch_opt']).to(self.device)
        else:
            self.cri_pix_gradBranch = None
    ...
    def setup_optimizers(self):
        ...
        if self.use_network_d_grad:
            # optimizer d
            optim_type = train_opt['optim_d_grad'].pop('type')
            self.optimizer_d_grad = self.get_optimizer(optim_type, self.net_d_grad.parameters(), **train_opt['optim_d_grad'])
    ...
    def train_net_g(self, current_iter):
        ...
        self.fake_H_branch, self.output, _ = self.net_g(self.lq)

        self.fake_H_grad = self.get_grad(self.output)
        self.var_H_grad = self.get_grad(self.gt)
        self.var_ref_grad = self.get_grad(self.ref)
        self.var_H_grad_nopadding = self.get_grad_nopadding(self.gt)
        ...
        if self.cri_pix_gradSR:
            l_pix_gradSR = self.cri_pix_gradSR(self.fake_H_grad, self.var_H_grad)
            self.l_g_total += l_pix_gradSR
            self.loss_dict['l_pix_gradSR'] = l_pix_gradSR

        if self.cri_pix_gradBranch:
            l_pix_gradBranch = self.cri_pix_gradBranch(self.fake_H_branch, self.var_H_grad_nopadding)
            self.l_g_total += l_pix_gradBranch
            self.loss_dict['l_pix_gradBranch'] = l_pix_gradBranch
    ...
    def optimize_parameters(self, current_iter):
        ...
        if not self.use_network_d:
            self.train_net_g(current_iter=current_iter)
        else:
            for p in self.net_d.parameters():
                p.requires_grad = False
            for p in self.net_d_grad.parameters():
                p.requires_grad = False
            if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
                ...
                if self.cri_gan_gradSR:
                    real_d_pred_grad = self.net_d_grad(self.var_ref_grad).detach()
                    fake_g_pred_grad = self.net_d_grad(self.fake_H_grad)
                    l_g_real_grad = self.cri_gan_gradSR(real_d_pred_grad - torch.mean(fake_g_pred_grad), False, is_disc=False)
                    l_g_fake_grad = self.cri_gan_gradSR(fake_g_pred_grad - torch.mean(real_d_pred_grad), True, is_disc=False)
                    l_g_gan_grad = (l_g_real_grad + l_g_fake_grad) / 2
                    self.l_g_total += l_g_gan_grad
                    self.loss_dict['l_g_gan_grad'] = l_g_gan_grad
            ...
            # real
            fake_d_pred_grad = self.net_d_grad(self.fake_H_grad).detach()
            real_d_pred_grad = self.net_d_grad(self.var_ref_grad)
            l_d_real_grad = self.cri_gan_gradSR(real_d_pred_grad - torch.mean(fake_d_pred_grad), True, is_disc=True) * 0.5
            l_d_real_grad.backward()
            # fake
            fake_d_pred_grad = self.net_d_grad(self.fake_H_grad.detach())
            l_d_fake_grad = self.cri_gan_gradSR(fake_d_pred_grad - torch.mean(real_d_pred_grad.detach()), False, is_disc=True) * 0.5
            l_d_fake_grad.backward()
            self.optimizer_d_grad.step()

            self.loss_dict['l_d_real_grad'] = l_d_real_grad
            self.loss_dict['l_d_fake_grad'] = l_d_fake_grad
            self.loss_dict['out_d_real_grad'] = torch.mean(real_d_pred_grad.detach())
            self.loss_dict['out_d_fake_grad'] = torch.mean(fake_d_pred_grad.detach())

        self.log_dict = self.reduce_loss_dict(self.loss_dict)

        del self.fake_H_branch
        del self.output
        del self.fake_H_grad
        del self.var_H_grad
        del self.var_ref_grad
        del self.var_H_grad_nopadding
    ...
    def tile_process(self, lq):
        ...
        output = self.img.new_zeros(output_shape)
                ...
                try:
                    with torch.no_grad():
                        if hasattr(self, 'net_g_ema'):
                            _, output_tile, _ = self.net_g_ema(input_tile)
                        else:
                            _, output_tile, _ = self.net_g(input_tile)
    ...
    def save(self, epoch, current_iter):
        ...
        if self.use_network_d_grad:
            self.save_network(self.net_d_grad, 'net_d_grad', current_iter)
