...
class MyRealESRGANModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(MyRealESRGANModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        if self.opt.get('Use_sharpen', None) is not None:
            ...
            self.gt, self.lq = paired_random_crop(img_gts = self.gt, img_lqs = self.lq, gt_patch_size = gt_size, scale = self.opt['scale'])
        # training pair pool
