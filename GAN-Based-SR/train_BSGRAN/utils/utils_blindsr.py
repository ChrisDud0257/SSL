...
def add_blur(img, sf=4):
    ...
    if random.random() < 0.3:
        ...
        k = anisotropic_Gaussian(ksize=2*random.randint(0,3)+3, theta=random.random()*np.pi, l1=l1, l2=l2)
    else:
        k = fspecial('gaussian', 2*random.randint(0,3)+3, wd*random.random())
...
def add_JPEG_noise(img):
    quality_factor = random.randint(75, 95)
    ...
    for i in shuffle_order:
        ...
        if i == 0:
            ...
            if random.random() < 0.75:
                sf1 = random.uniform(1, 1/0.85)
                img = cv2.resize(img, (int(1/sf1*img.shape[1]), int(1/sf1*img.shape[0])), interpolation=random.choice([1,2,3]))
            else:
                k = fspecial('gaussian', 2*random.randint(0,3) + 3, random.uniform(0.1, 0.25*sf))
            ...
            # add Gaussian noise
            img = add_Gaussian_noise(img, noise_level1=1, noise_level2=12)
...
def degradation_bsrgan_nocrop(img, sf=4, lq_patchsize=72, isp_model=None):
    """
    This is the degradation model of BSRGAN from the paper
    "Designing a Practical Degradation Model for Deep Blind Image Super-Resolution"
    ----------
...
def degradation_bsrgan_ours_p(img, img_E, sf=4, lq_patchsize=72, isp_model=None):
    """
    This is the degradation model of BSRGAN from the paper
    "Designing a Practical Degradation Model for Deep Blind Image Super-Resolution"
    ----------
    ...
    # random crop
    img, hq, hq_E = random_crop_ours_p(img, hq, hq_E, sf_ori, lq_patchsize)
